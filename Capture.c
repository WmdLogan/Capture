...
void cap_analysis() {
    while (1) {
        int m;
        struct pcap_pkthdr *de_packet_header = (struct pcap_pkthdr *) malloc(sizeof(struct pcap_pkthdr));
        u_char *de_packet_content = (u_char *) malloc(sizeof(u_char) * 1600);

        pthread_mutex_lock(&queue_mutex);
        m = de_queue(cap_queue, de_packet_header, de_packet_content);
        pthread_mutex_unlock(&queue_mutex);

        if (m) {
            bpf_u_int32 len = de_packet_header->len;
...
//只要IP协议的包
            ethernet_protocol = (struct ether_header *) de_packet_content;
            if (ntohs(ethernet_protocol->ether_type) == 0x0800) {
                ip_protocol = (struct ip_header *) (de_packet_content + 14);
...
//如果IP符合，获取端口
                {
                    ...
                    switch (ip_protocol->ip_protocol) {
                        case 6:
                            ...
                            tcp_protocol = (struct tcp_header *) (de_packet_content + 14 + 20);
                        ...
                        case 17:
                            ...
                            udp_protocol = (struct udp_header *) (de_packet_content + 14 + 20);
...
//插入哈希表
                        ...
                        insert_hash(de_packet_content, TCAP_hash, len);
...
//计算如果保存这个包，文件的大小
                        printf("Received Packet Size: %d\n", de_packet_header->len);
                        current_size += de_packet_header->len + 16;
                        printf("current size is = %d\n", current_size);
//如果文件大小、时间，如果符合，保存
                        if (current_size < atoi(file_size) && time(&rawtime) - cap_buf.st_ctime < file_time) {
                            pcap_dump((u_char *) out_pcap, de_packet_header, de_packet_content);
...
//开始记录修改时间
                            stat(final_path, &cap_buf);
                            printf("new file time: %ld\n", cap_buf.st_ctime);
                            current_size = de_packet_header->len + 40;
                        }
                    }
                }
            }
        ...
        free(de_packet_header);
        free(de_packet_content);
    }
}