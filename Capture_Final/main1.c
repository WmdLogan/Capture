...
int netinterface_flag = 1;//update线程首次配置结束标志
//检查配置文件更新线程
void check_update() {
...
//                    printf("配置file_time为%d\n", file_time);
            ...
            strcpy(des_add, inet_ntoa(d_addr));
            printf("%s\n", des_add);
            if (break_loop_flag)  pcap_breakloop(pcap_handle);
        }
    }
}
...
//网卡修改，需要重新给pcap设置网卡
    if (strcmp(net_interface, "") != 0) {
        ...
        pcap_lookupnet("ens5", &net_ip, &net_mask, error_content);
        pcap_handle = pcap_open_live("ens5", BUFSIZ, 1, 1, error_content);
    }
